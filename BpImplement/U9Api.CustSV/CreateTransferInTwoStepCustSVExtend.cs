		...
		public override object Do(object obj)
		{
			...
			using (UFSoft.UBF.Transactions.UBFTransactionScope scope = new UFSoft.UBF.Transactions.UBFTransactionScope(UFSoft.UBF.Transactions.TransactionOption.RequiresNew))
			{
				try
				{
					...
					{
						item.PlanRcvDate = businessDate;
						item.DocDate = businessDate;
					}
					...
					using (ISession session = Session.Open())
					{
						...
						doc.Status = TransInStatus.Opening;

						doc.CurrAction = TransferInActionEnum.UIUpdate;
						session.Commit();
					}
					TransferIn doc2 = TransferIn.Finder.FindByID(listRes[0]);
					...
					scope.Commit();
					return JsonUtil.GetSuccessResponse(doc2.DocNo, doc2.Status.Name, debugInfo);
				}
				catch (Exception ex)
				{
